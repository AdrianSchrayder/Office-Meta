...
let BattleAbilities = {
	...
	"battery": {
		shortDesc: "This Pokemon's electric attacks are multiplied by 1.5.",
	...
	"battlearmor": {
		shortDesc: "This Pokemon's Attack and Defense is raised by 1 stage after it is damaged by a move.",
	...
	"blaze": {
		...
		onModifyMove(move, pokemon) {
			if (move.type === 'Normal' && !['judgment', 'multiattack', 'naturalgift', 'revelationdance', 'technoblast', 'weatherball'].includes(move.id) && !(move.isZ && move.category !== 'Status')) {
				move.type = 'Fire';
				move.blazeBoosted = true;
			}
		},
		onBasePowerPriority: 8,
		onBasePower(basePower, pokemon, target, move) {
			if (move.blazeBoosted) return this.chainModify(1.5);
		},
		id: "blaze",
		name: "Blaze",
		rating: 2.5,
	...
	"heavymetal": {
		...
		onSourceModifyAtk(atk, attacker, defender, move) {
			if (move.type === 'Ground' || move.type === 'Fighting') {
	...
	"honeygather": {
		shortDesc: "Pokemon recovers 1/8 health at the end of every turn.",
		id: "honeygather",
		onResidualOrder: 5,
	...
	"ironfist": {
		...
		shortDesc: "This Pokemon's punch-based attacks have 1.5x power. Sucker Punch is boosted.",
		onBasePowerPriority: 8,
		onBasePower(basePower, attacker, defender, move) {
			if (move.flags['punch']) {
	...
	"overgrow": {
		...
		onModifyMove(move, pokemon) {
			if (move.type === 'Normal' && !['judgment', 'multiattack', 'naturalgift', 'revelationdance', 'technoblast', 'weatherball'].includes(move.id) && !(move.isZ && move.category !== 'Status')) {
				move.type = 'Grass';
				move.overgrowBoosted = true;
			}
		},
		onBasePowerPriority: 8,
		onBasePower(basePower, pokemon, target, move) {
			if (move.overgrowBoosted) return this.chainModify(1.5);
		},
		id: "overgrow",
		name: "Overgrow",
		rating: 2.5,
	...
	"steadfast": {
		shortDesc: "If this Pokemon flinches, all its stats are raised by 1 stage.",
		onFlinch(pokemon) {
			this.boost({atk: 1});
			this.boost({def: 1});
			this.boost({spa: 1});
	...
	"swarm": {
		...
		onModifyMove(move, pokemon) {
			if (move.type === 'Normal' && !['judgment', 'multiattack', 'naturalgift', 'revelationdance', 'technoblast', 'weatherball'].includes(move.id) && !(move.isZ && move.category !== 'Status')) {
				move.type = 'Bug';
				move.swarmBoosted = true;
			}
		},
		onBasePowerPriority: 8,
		onBasePower(basePower, pokemon, target, move) {
			if (move.swarmBoosted) return this.chainModify(1.5);
		},
		id: "swarm",
		name: "Swarm",
		rating: 2.5,
	...
	"torrent": {
		...
		onModifyMove(move, pokemon) {
			if (move.type === 'Normal' && !['judgment', 'multiattack', 'naturalgift', 'revelationdance', 'technoblast', 'weatherball'].includes(move.id) && !(move.isZ && move.category !== 'Status')) {
				move.type = 'Water';
				move.torrentBoosted = true;
			}
		},
		onBasePowerPriority: 8,
		onBasePower(basePower, pokemon, target, move) {
			if (move.torrentBoosted) return this.chainModify(1.5);
		},
		id: "torrent",
		name: "Torrent",
		rating: 2.5,
